...
public final class ReflectiveTypeAdapterFactory implements TypeAdapterFactory  {
  ...
  // Implémentation de la méthode pour TypeAdapterFactoryInterface
  @Override
  public <T> Object create(Object gson, Type type) {
    return create((Gson) gson, TypeToken.get(type));
  }
  ...
  private BoundField createBoundField(
    ...
    if (annotation != null) {
      ...
      mapped =
              jsonAdapterFactory.getTypeAdapter(
                      constructorConstructor, (Gson) context, fieldType, annotation, false);
    }
    boolean jsonAdapterPresent = mapped != null;
    if (mapped == null) {
      mapped = ((Gson) context).getAdapter(fieldType);
    }
    ...
    if (serialize) {
      writeTypeAdapter =
              jsonAdapterPresent
                      ? typeAdapter
                      : new TypeAdapterRuntimeTypeWrapper<>((Gson) context, typeAdapter, fieldType.getType());
    } else {
  ...
  private FieldsData getBoundFields(
          JsonProcessor context, TypeToken<?> type, Class<?> raw, boolean blockInaccessible, boolean isRecord) {
    if (raw.isInterface()) {
